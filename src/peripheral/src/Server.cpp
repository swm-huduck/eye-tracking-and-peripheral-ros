	...
	GGKServerDataGetter getter, GGKServerDataSetter setter)
{
	...
	.gattServiceBegin("text", "00000001-1E3C-FAD4-74E2-97A033F1BFAA")
		...
		.gattCharacteristicBegin("string", "00000002-1E3C-FAD4-74E2-97A033F1BFAA", {"read", "write", "notify"})
			...
			.onWriteValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				...
				// self.setDataPointer("text/string", pParameters);
				// const char * str = Utils::stringFromGVariantByteArray(pAyBuffer).c_str();
				// q.push(str);
				...
				if((int)array[0] == 1) {
					...
					self.setDataPointer("text/string", result.data());
				}
	...
	// .gattServiceBegin("a", "CB660002-4339-FF22-A1ED-DEBFED27BDB4")
	// 	.gattCharacteristicBegin("b", "CB660004-4339-FF22-A1ED-DEBFED27BDB4", {"read", "write"})
	// 		.onWriteValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA{
	// 			// Update the text string value
	// 			GVariant *pAyBuffer = g_variant_get_child_value(pParameters, 0);
	...
	// 			// dbus "method_return" is sent, otherwise the client gets an error (ATT error code 0x0e"unlikely").
	// 			// Only "write-without-response" works without this
	// 			self.methodReturnValue(pInvocation, 0, true);
	// 			// self.methodReturnVariant(pInvocation, NULL, true);
	// 		})
	// 		.onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA{

	// 		})
	// 		.gattDescriptorBegin("n", "00005609-0000-1001-8080-00705c9b34cb", {"read", "write"})
	// 			.onReadValue(DESCRIPTOR_METHOD_CALLBACK_LAMBDA {
	// 				self.methodReturnValue(pInvocation, 0, true);
	// 			})
